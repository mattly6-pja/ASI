...
# diabetes predictor
model = joblib.load("data/06_models/diabetes/model_raw_diabetes.pkl")
preprocessor = joblib.load("data/06_models/diabetes/preprocessor_diabetes.pkl")

st.title("Sprawdź jakie jest Twoje ryzyko zawału (już dziś)!")

gender_dict = {"Mężczyzna": "Male", "Kobieta": "Female"}
smoking_dict = {
    "Nigdy nie paliłem/am": "non-smoker",
    "Paliłem/am w przeszłości, ale obecnie nie palę": "smoker",
    "Palę obecnie": "smoker",
}
binary_dict = {"Nie": 0, "Tak": 1}
...
hba1c = st.number_input("Poziom hemoglobiny glikowanej (HbA1c) w % - jeśli nieznany, pozostaw puste", 4.0, 15.0, 7.0)
hba1c = hba1c if hba1c else 5.0
...
transformed = pd.DataFrame(
    preprocessor.transform(input_data),
    columns=[
        *preprocessor.named_transformers_["num"].get_feature_names_out(["age", "blood_glucose_level", "bmi", "HbA1c_level"]),
        *preprocessor.named_transformers_["cat"].get_feature_names_out(
...
if st.button("Predict Stroke Risk"):
    ...
    feature_values = transformed.iloc[0].values
    all_features = [(feature_names[i], coefficients[i] * feature_values[i]) for i in range(len(feature_names))]
    top_features = sorted(all_features)[:3]
